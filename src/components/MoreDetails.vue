...
export default {
  ...
  methods: {
    getMoreDetails() {
      ...
      axios
        .get(
          `${import.meta.env.VITE_BASE_URL}/${this.tvOrMovie}/${this.id}?api_key=${import.meta.env.VITE_API_KEY}&language=pt-BR&append_to_response=images,videos,recommendations,reviews,credits&include_image_language=en,null&include_video_language=pt-BR`
        )
        .then((response) => {
